...
const Drawer = ({ children, setIsOpen = (v: boolean) => {}, isOpen = false }) => {
  ...
  return (
    <>
      ...
      <div
        id="maxZindex"
        className={`p-8 fixed inset-y-0 left-0 bg-white xl:w-[45%] md:w-[70%] w-[90%] shadow-lg transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
