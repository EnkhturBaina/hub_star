...
const CatItem: React.FC<IProps> = ({ mainDirection = null, queryId = null }) => {
  ...
  return (
    <>
      <div
        className={`cursor-pointer bg-gradient-to-b shadow-solid-8 w-full lg:h-36 h-25 !flex-none hover:bg-gradient-to-t hover:from-mainColor hover:to-transparent ${Number(queryId) === id ? 'bg-gradient-to-t from-mainColor to-transparent' : ''}`}
      >
        <Link
